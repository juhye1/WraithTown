...
public class EnemyChaseState : BaseMoveState
{
    ...
    {
        base.Enter();
        fsm.rb.velocity = (fsm.targetPos - (Vector2)fsm.transform.position).normalized * fsm.enemy.stat.move_speed;
    ...
    protected void MoveTowardsTarget(IState newState)
    {
        ...
        {
            fsm.rb.velocity = (fsm.player.transform.position - fsm.transform.position).normalized * fsm.enemy.stat.move_speed;
